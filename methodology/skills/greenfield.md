...
If you're here because of an interrupted session (`.claude/shipwright.md` has `status: in-progress`
and `project_type: greenfield`):

1. Check what already exists — package.json, src/, .gitignore, dependencies installed?
2. Read the `notes` field in `.claude/shipwright.md` for decisions already made (stack, framework, etc.)
